...
function alphaSymbol(symbol: string, exchange: Exchange) {
  return exchange === "BSE" ? `BSE:${symbol}` : `NSE:${symbol}`;
}
...
export async function fetchAlphaVantageQuote(
  ...
  url.searchParams.set("symbol", alphaSymbol(clean, exchange));
    ...
    return {
      ...
      warnings: [
        data.Note || data.Information
          ? "Alpha Vantage rate limit or coverage notice returned"
          : "Alpha Vantage coverage can be limited for Indian equities"
      ]
    };
  } catch {
    return null;
